...
TestLightB::TestLightB(const std::string& name /*= "Layer"*/)
	: m_Camera(60.0f, 16.0f / 9.0f)
{
	m_Camera.GetCamera().SetPosition({ 0, 0, 3 });
}

TestLightB::~TestLightB()
{}

void TestLightB::OnAttach()
{
	m_ObjectShader = std::make_unique<Shader>("layers/TestLightB/res/bLightVertB.glsl", "layers/TestLightB/res/bLightFragB.glsl");
	m_LightShader = std::make_unique<Shader>("layers/TestLightB/res/basicVertex.glsl", "layers/TestLightB/res/basicFragment.glsl");
...
void TestLightB::OnDetach()
{
	GLCall(glDisable(GL_DEPTH_TEST));
}

void TestLightB::OnUpdate(float dt)
{
	if(m_CameraEnabled)
	...
	{	
		...
		m_ObjectShader->SetUniform3f("u_LightPos", m_Light.position);
	...
	{
		m_Light.position.x = 1 + sin(glfwGetTime()) * 2.f;
		m_Light.position.y = sin(glfwGetTime() / 2);
		glm::mat4 MVP = m_Camera.GetCamera().GetViewProjectionMatrix() * glm::translate(glm::mat4(1.0f), m_Light.position) * glm::scale(glm::mat4(1.0f), glm::vec3(0.2f));
		m_LightShader->Bind();
...
void TestLightB::OnEvent(Event& event)
{
